...
spots = (
    Spot("s1", "z1", 1, SpotStatus.AVAILABLE, SpotKind.STANDARD, []),
    Spot("s2", "z2", 2, SpotStatus.AVAILABLE, SpotKind.TRUCK, []),
    Spot("s3", "z2", 3, SpotStatus.AVAILABLE, SpotKind.ACCESSIBLE, []),
    Spot("s4", "z3", 4, SpotStatus.AVAILABLE, SpotKind.ACCESSIBLE, []),
    Spot("s5", "z4", 5, SpotStatus.AVAILABLE, SpotKind.STANDARD, []),
)
...
def test_zone_id_filter_1():
    ...
    assert reduce(lambda acc, spot: acc and spot.zone_id == "z2", testing_by_zone("z2"))
...
def test_time_range_3():
    ...
    """
    testing_by_time_range = make_by_time_range(sessions)
    assert not testing_by_time_range(
        date("1941-09-01T04:00:00"), date("1945-05-9T09:07:00")
    )
